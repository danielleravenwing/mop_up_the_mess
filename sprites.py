...
class Player(pg.sprite.Sprite):
    def __init__(self, game, x, y, kind = 'blue', player_num = 10):
        ...
        self.kind = kind
        if self.kind == 'blue':
            self.game.group_cols[2].append(self)
            self.image = self.game.blue_mop_image.copy()
            self.mop_rate = 500
            self.player_acc = PLAYER_ACC
        elif self.kind == 'red':
            self.game.group_cols[1].append(self)
            self.image = self.game.red_mop_image.copy()
            self.mop_rate = 300
            self.player_acc = PLAYER_ACC + 0.25
        elif self.kind == 'yellow':
            self.game.group_cols[3].append(self)
            self.image = self.game.yellow_mop_image.copy()
            self.mop_rate = 100
            self.player_acc = PLAYER_ACC + 0.5
        elif self.kind == 'green':
            self.game.group_cols[0].append(self)
            self.image = self.game.green_mop_image.copy()
            self.mop_rate = 1
            self.player_acc = PLAYER_ACC + 1
        self.blink_speed = 100
        self.open_eyes = 2000
        self.last_animate = 0
        self.refreash_animate = True
        self.blink_image = self.game.blink_image.copy()
        self.frame = 0
        self.animation = []
    ...
    def kills(self, value):
        self._kills = value
        if value > (10 + self.game.level) * 3:
            self.change_color()

    @property
    def player_num(self):  # This is the method that is called whenever you access
        return self._player_num
    @player_num.setter # Runs whenever you set a value
    ...
    def add_accessory(self):
        if self.powerup_kind not in ['bleach', 'peroxide']:
            image_name = "self.game." + self.kind + "_mop_image"
            self.image = eval(image_name).copy()
        else:
            self.image = self.game.bleach_mop_image.copy()
        if self.player_num > 4:
            pass
        else:
            i = self.player_num
            surface = pg.Surface((self.image.get_width(), self.image.get_width()), pg.SRCALPHA).convert_alpha()
            surface.blit(self.image, (0, 0))
            surface.blit(self.game.accessorys[i], (0, 0))
            self.image = surface
        self.refreash_animate = True
    ...
    def change_color(self, color = None):
        ...
        #reassigns colored players to correct groups
        self.game.group_cols = []
        cols = ['green', 'red', 'blue', 'yellow']
        for color in range(0, 4):
            glist = []
            for player in self.game.players:
                if player.kind == cols[color]:
                    glist.append(player)
            self.game.group_cols.append(glist)
    ...
    def get_powerup(self, kind):
        ...
        if kind in ['bleach', 'peroxide']:
            ...
            sound = 'itempickup'
            self.game.channel6.stop()
            self.game.channel6.play(self.game.effects_sounds[sound])
    ...
    def apply_forces(self):
        ...
        if hits:
            ...
            self.rect.centerx = self.pos.x
            self.rect.centery = self.pos.y
            dv.x += hits[0].vel.x / 10
            dv.y += hits[0].vel.y / 10
        return dv

    def update(self):
        self.acc = vec(0, 0)
        if self.clicked:
            if self.player_num == self.game.num_joysticks:
                ...
                if mouse_movement > 0:
                    ...
                    except: # Avoid scaling a zero vector.
                        pass
        forces = self.apply_forces()
        self.acc.x += self.vel.x  * PLAYER_FRICTION + forces.x
        self.acc.y += self.vel.y * PLAYER_FRICTION + forces.y
        self.vel += self.acc
    ...
    def animate(self):
        if self.refreash_animate:
            if self.player_num < 5:
                frame0 = self.image.copy()
                surface = pg.Surface((self.image.get_width(), self.image.get_width()), pg.SRCALPHA).convert_alpha()
                surface.blit(frame0, (0, 0))
                surface.blit(self.blink_image, (0, 0))
                surface.blit(self.game.accessorys[self.player_num], (0, 0))
                frame1 = surface
                self.animation = [frame0, frame1]
                self.refreash_animate = False
            else:
                frame0 = self.image.copy()
                surface = pg.Surface((self.image.get_width(), self.image.get_width()), pg.SRCALPHA).convert_alpha()
                surface.blit(frame0, (0, 0))
    ...
    def drop_bomb(self):
        if self.bombs > 0:
            self.bombs -= 1
            if self.bombs < 0: self.bombs = 0
            PowerUp(self.game, self.pos.x, self.pos.y, 'bomb')
...
class PowerUp(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.triggered:
            now = pg.time.get_ticks()
            if now - self.set_time > self.fuse:  # Makes bombs explode after you trigger them.
                Explosion(self.game, self.rect.center)
                self.kill()

    def hit(self, player = None):
        if self.kind in ['bleach', 'peroxide', 'bombbox']:
            ...
            if self.triggered:
                return
            self.triggered = True
            self.set_time = pg.time.get_ticks()
            oldx = self.rect.x
            oldy = self.rect.y
            self.image = self.game.set_bomb_image
            self.rect = self.image.get_rect()
            self.rect.x = oldx
            self.rect.y = oldy
...
class GiantSlime(pg.sprite.Sprite):
    ...
    def move(self):
        ...
        if (self.target_tile - self.pos).length() <= self.size:
            # Randomly selects a target tile location if the slime has reached the target location, or is very close.
            border = int(self.size / (2*TILE_SIZE))
            xtarget = randrange(border, self.game.xtiles - border) * TILE_SIZE
            ytarget = randrange(border, self.game.ytiles - border) * TILE_SIZE
    ...
    def choose_jump_target(self):
        if not self.jump_up:
            targetx = randrange(self.size//2, int(WIDTH - self.size/2))
            targety = randrange(self.size//2, int(HEIGHT - self.size - (2 * TILE_SIZE)))
            self.jump_up = True
        else:
